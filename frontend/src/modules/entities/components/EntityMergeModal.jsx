...
export default function EntityMergeModal({ folderId, open, onClose, onMerged }) {
  ...
  const [selection, setSelection] = useState({}); // groupIndex -> { targetId, sourceIds: [] }
  ...
  const handleChooseTarget = (gi, id) => {
    setSelection((s) => ({ ...s, [gi]: { targetId: id, sourceIds: (s[gi]?.sourceIds || []).filter((x) => x !== id) } }));
  };
  ...
  const handleToggleSource = (gi, id) => {
    setSelection((s) => {
      ...
      return { ...s, [gi]: { ...prev, sourceIds: next.filter((x) => x !== prev.targetId) } };
    });
  };
  ...
  const doMerge = async (gi) => {
    ...
    setSelection((s) => { const { [gi]: _, ...rest } = s; return rest; });
  };
      ...
      <div className="space-y-4">
        {groups.map((g, gi) => (
          <div key={gi} className="border rounded p-3">
            <div className="font-semibold mb-2">Groupe #{gi + 1} — score moyen {g.score}</div>
                ...
                return (
                  <div key={c.id} className="flex items-center justify-between border rounded px-2 py-1">
                    <div className="truncate">
                      <div className="text-sm font-medium">[{c.type}] {c.name || '(sans nom)'} — #{c.id}</div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Button variant={isTarget ? 'primary' : 'secondary'} onClick={() => handleChooseTarget(gi, c.id)}>
                        {isTarget ? 'Cible ✓' : 'Choisir cible'}
                      </Button>
                      <Button variant={isSource ? 'danger' : 'secondary'} disabled={sel.targetId === c.id} onClick={() => handleToggleSource(gi, c.id)}>
            ...
            <div className="mt-3 flex justify-end gap-2">
              <Button disabled={!canMerge(gi)} onClick={() => doMerge(gi)}>Fusionner le groupe</Button>
            </div>
          </div>
        ))}
      </div>
